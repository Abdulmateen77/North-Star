...
echo
echo "=== GET /api/care-spaces, no token (expect 401 UNAUTHORIZED) ==="
curl -s -w " HTTP:%{http_code}\n" "$BASE/api/care-spaces" --max-time 20

echo
echo "=== GET /api/care-spaces, malformed token (expect 401 UNAUTHORIZED) ==="
curl -s -w " HTTP:%{http_code}\n" \
  -H "Authorization: Bearer not-a-real-token" \
  "$BASE/api/care-spaces" --max-time 20

echo
echo "=== GET /api/dashboard, no token (expect 401 UNAUTHORIZED) ==="
