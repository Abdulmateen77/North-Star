  ...
  return (
    <div className="animate-fade-up flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        {eyebrow ? (
          <p className="text-xs font-medium tracking-wide text-olive-400 uppercase">{eyebrow}</p>
        ) : null}
        <h1 className="mt-1.5 text-3xl leading-tight text-balance text-olive-900 sm:text-4xl">
          {title}
        </h1>
        {description ? (
          <p className="mt-2.5 max-w-2xl leading-relaxed text-pretty text-olive-600">
            {description}
          </p>
        ) : null}
      </div>
