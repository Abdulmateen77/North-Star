      ...
      <section className="mt-9">
        ...
        <ul className="mt-3 space-y-3">
          {documents.map((doc) => (
            ...
            >
              <p className="text-lg leading-snug font-medium text-olive-900">{doc.title}</p>
              <p className="mt-0.5 text-sm text-olive-400">
                {doc.source} · {doc.dateLabel}
              </p>

              {doc.aiSummary !== null ? (
                <div className="mt-3.5 rounded-2xl border border-gold-100 bg-gold-50/60 p-4">
                  <p className="flex items-center gap-2 text-xs font-medium tracking-wide text-gold-500 uppercase">
                    <StarMark size={12} />
                    In plain English
                  </p>
                  <p className="mt-2 leading-relaxed text-pretty text-olive-800">
                    {doc.aiSummary}
                  </p>
                </div>
              ) : null}
